...
/**
 * This algorithm merges best parts of different approaches tested during
 * project.
 * 
...
public class ComputationEngineImpl implements ComputationEngine {
	...
	/**
	 * Find smallest possible group of change initializers to achieve breakpoint and
	 * spread new preference in major part of graph It uses heuristics that vertices
	 * with more neighbors are more influential.
	 * 
	...
	private void orderList() {
		...
		Collections.sort(initialList, (a, b) -> b.getNeighbors().size() - a.getNeighbors().size());
	...
	private void forceChangeToFalse() {
		...
		if (totalTrues > 3) {
			...
			forceToFalseEdgeCase();
		}
		descNumToChange();
	}

	private void forceToFalseEdgeCase() {
		
		totalTrues--;
		nextIndexToChange--;
		preferencesMap[nextIndexToChange] = false;
	}

	private void forceToFalseStdCase() {
	...
	/**
	 * Decreasing next part of force converts
	 */
	...
	/**
	 * Converting major part of vertices. It's efficient and precise, but leaves
	 * "stubborn group"
	 * 
	 * @param threshold
	 */
	private void computeRoundOne(int threshold) {
		...
		while (numToChange > 0 && alreadyComputing.get()) {
	...
	private void computeRoundWithStreams() {
		...
		initialList.parallelStream().forEach(vertex -> makeDecission(vertex));
		initialList.parallelStream().forEach(vertex -> vertex.switchPreference());
	...
	private boolean isLoopNotInPeriod(int[] periodChecker, int i) {
		...
		return (i >= 3
				&& (periodChecker[i - 1] != periodChecker[i - 2] || periodChecker[i - 1] != periodChecker[i - 3]))
				|| i < 3;
	}
	...
	/**
	 * Decides if we should add or remove some trues before next round of
	 * computation
	 */
